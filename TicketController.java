...
public class TicketController {
    ...
    public ResponseEntity<ApiResponseDto<Ticket>> bookTickets(
        ...
        if (principal instanceof User) {
            userLoginId = ((User) principal).getLoginId();
        } else if (principal instanceof UserDetails) {
            userLoginId = ((UserDetails) principal).getUsername();
        } else {
    ...
    public ResponseEntity<ApiResponseDto<Ticket>> getTicketByReference(
        ...
        if (!user.isAdmin() && !ticket.getUserLoginId().equals(user.getLoginId())) {
    ...
    public ResponseEntity<ApiResponseDto<List<Ticket>>> getUserTicketsByUsername(
        ...
        if (!currentUser.isAdmin() && !currentUser.getLoginId().equalsIgnoreCase(username)) {
            throw new com.moviebookingapp.exception.AuthorizationException(
                "You can only view your own tickets");
        }
        
        logger.info("Request received to fetch tickets for username: {} by user: {}", username, currentUser.getLoginId());
